...
our $VERSION = 			'$Revision: 527 $';
our $LAST_CHANGED_DATE =	'$LastChangedDate: 2013-05-21 08:53:41 -0700 (Tue, 21 May 2013) $';

our ($verbose, $help, $man);
our ($queryfile, $dbloc);
our ($outfile, $buildver, $remove, $checkfile, $protocol, $operation, $otherinfo, $onetranscript, $sortout, $nastring, $genericdbfile, $gff3dbfile, $bedfile, $vcfdbfile, $csvout, $argument);
our ($colswanted);
...
GetOptions('verbose|v'=>\$verbose, 'help|h'=>\$help, 'man|m'=>\$man, 'outfile=s'=>\$outfile, 'buildver=s'=>\$buildver, 'remove'=>\$remove, 'checkfile!'=>\$checkfile, 
	...
	'csvout'=>\$csvout, 'argument=s'=>\$argument,'colsWanted=i'=>\$colswanted,
	) or pod2usage ();
...
sub printSortOutput {
	...
	for my $var(&chr_sort (keys %varanno)) {
		...
		for my $item (@header) {
			my $num_ljb_field = 11;
			$item eq 'ljb2_all' and $num_ljb_field = 15;
			if ($csvout) {
				if ($item =~ m/^ljb_all$/) {
					push @oneline,($varanno{$var}{$item} || join (",", ($nastring) x $num_ljb_field));	#value is already comma-delimited
				} else {
					push @oneline,(defined $varanno{$var}{$item}) ? qq/"$varanno{$var}{$item}"/ : $nastring;
				}
			} else {
				if ($item =~ m/^ljb\d*_all$/) {
					if ($varanno{$var}{$item}) {
...
sub printOrigOutput {
	...
	while (<FH>) {
		...
		for my $item (@header) {
			...
			if ($csvout) {
				if ($item =~ m/^ljb\d*_all$/) {
					push @oneline,($varanno{$varstring}{$item} || join (",", ($nastring) x $num_ljb_field));	#value is already comma-delimited
				} else {
...
sub processArgument {
	...
	if (not $protocol) {
		...
		if ($buildver eq 'hg18') {
			...
			print STDERR "NOTICE: the --protocol argument is set as 'gene,phastConsElements44way,genomicSuperDups,esp6500si_all,1000g2012apr_all,snp137,ljb_all' by default\n";
...
sub regionOperation {
	...
	my ($userfile, $header);
	my $sc = "annotate_variation.pl -regionanno ".($colswanted? " -colsWanted $colswanted ":"")." -dbtype $dbtype -buildver $buildver -outfile $outfile $queryfile $dbloc";
	$argument and $sc .= " $argument";
...
=head1 SYNOPSIS
 ...
 Optional arguments:
	    ...
	    --colsWanted <int>		columns wanted to output, separated by comma
