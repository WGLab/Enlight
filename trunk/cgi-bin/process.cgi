...
&error ("Too many generic tracks") and exit 0 if @generic_table > $generic_table_max;
&error ("Incorrect security code, you should type \'$dateSecurity\'.") and exit 0 if $securityCode ne $dateSecurity; #use day number as security code

#debug consider DoS attack

my $dsn="DBI:mysql:database=$dbname";
my $dbh=DBI->connect($dsn,$dbuser,$dbpassword) or die "Cannot connect: $DBI::errstr\n";
...
sub jobRegister
{
	...
	my $access=&rndStr(8,'a'..'z',0..9);
	my ($ld_source,$ld_ref,$ld_pop)=split(',',$source_ref_pop);
	&error("Genome builds don't match ($ref vs $source_ref_pop).") and exit 0 unless $source_ref_pop =~ /$ref/i;
	my $param;
