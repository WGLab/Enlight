...
if ($pg=$k) {
$page .= qq(<a href="javascript:fm.submit();" onclick=document.getElementById("fname").value=$k><b>$k<b></a>&nbsp;);
}
else{
...
print qq($heade<body class="results"><p>100 results per page</p><p><a href="http://wannovar.usc.edu/">back to HOME</a></p>$page<pre>$title)."<tbody>".qq($content</tbody></table></pre><p>$page</p><hr><form method="post" name=fm action=http://wannovar.usc.edu/cgi-bin/web_result_filter.pl><h2>$form</select></h2><br>$filter</form></body></html>);
