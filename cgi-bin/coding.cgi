...
#			1;
#		} else {
#			exec("/var/www/html/annovar-server/exec/control_annovar.pl $submission_id") or confess "can not execute submission";       
#		}
#	}
}
...
close (INFO);
if (($ip and $ip eq $info{"ip"}) and ($host eq $info{"host"})){
$cnt++;
}
if ($cnt>=4) {
