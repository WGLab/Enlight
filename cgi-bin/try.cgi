...
my $python_dir=$server_conf{'python_bin'} || &Utils::error("No Python 2.7 executable directory\n",$log,$admin_email);
...
my $custom_fh=$q->upload('custom_table');
#die ($q->cgi_error) if ($q->cgi_error);
#die ("ERROR: no custom table\n") unless $custom_fh;
my @custom_table=$q->param("custom_table");
#my @custom_table_files=$q->tmpFileName(@custom_table);

#debug !!!
map {print "CUSTOM: $_!!!!!".$q->tmpFileName($_)."</br>"} @custom_table;
exit;
...
map { print "0000generic table: $_</br>" } @generic_table;
print "table names: @custom_table\n";
print "table files: ",$q->tmpFileName($custom_table[0]),$q->tmpFileName($custom_table[1]),"\n";
print "chr",$q->param('chr'),"start",$q->param('start'),"snp",$q->param('refsnp'),"gene",$q->param('refgene'),"\n";
